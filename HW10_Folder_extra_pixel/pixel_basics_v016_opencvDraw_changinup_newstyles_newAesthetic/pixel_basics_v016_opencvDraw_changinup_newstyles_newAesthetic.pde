...
void draw() {
  ...
  for (int i = 0; i<faces.length; i++) {
    ...
    for (int z = 0; z < 600; z += 2) {
      ...
      color blue = color(244, 100, 0, 100);
      color white = color(244, 244, 244, 100);
      ...
      line(x, y, x+map(brightness(c), 0, 255, 10*cos(PI/4), 4*cos(-PI/4)), y+map(brightness(c), 0, 255, 10*sin(PI/4), 4*sin(-PI/4)));
    }
    //  }
  }
  rectMode(CENTER);
