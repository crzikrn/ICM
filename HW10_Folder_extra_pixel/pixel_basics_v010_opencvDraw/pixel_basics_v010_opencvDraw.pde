...
void draw() {
  ...
  for (int i = 0; i<faces.length; i++) {
    ...
    noFill();
    Rectangle faced = new Rectangle(faces[i].x, faces[i].y, faces[i].width+30, faces[i].height+30);
    //noStroke();
    //rect(faces[i].x, faces[i].y, faces[i].width, faces[i].height);
    //fill(244);
    //rect(0,0,width,height);
    for (int z = 0; z < 6000; z += 1) {
      ...
      line(x, y, x+map(brightness(c), 0, 255, 8*cos(50), 8*sin(-10)), y+map(brightness(c), 0, 255, 8*cos(50), 8*sin(-1)));
    }
  }
}
