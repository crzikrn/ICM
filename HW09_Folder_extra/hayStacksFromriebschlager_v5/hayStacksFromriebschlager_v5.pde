...
void draw() {
  ...
  noStroke();
  background(244);
  //put capture image behind
  //image(cam,0,0);
  
  //controlling the lines or rectangles
  for (int i = 0; i < cam.pixels.length; i+=5) {
    ...
    //Color range where bright becomes white and dark becomes blue
    ...
    float newLength = map(nX, 0, height, 2, 40);
    float newWidth = map(nX, 0, height, 0, 1);
    rectMode(CENTER);
